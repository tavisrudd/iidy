...
pub async fn load_ssm_import_with_client(
    location: &str,
...
pub async fn load_ssm_path_import(
    location: &str,
    aws_config: &aws_config::SdkConfig,
) -> Result<ImportData> {
    let client = AwsSsmClient::new(aws_config);
...
pub async fn load_ssm_path_import_with_client(
    location: &str,
    client: &dyn SsmClient,
) -> Result<ImportData> {
    ...
    for parameter in parameters {
        ...
        let parsed_value = match format.as_deref() {
            Some("json") => serde_json::from_str(&parameter.value)
                .unwrap_or(serde_json::Value::String(parameter.value)),
            Some("yaml") => match serde_yaml::from_str::<serde_json::Value>(&parameter.value) {
                Ok(value) => value,
...
mod tests {
    ...
    async fn test_load_ssm_import_string() -> Result<()> {
        let client = MockSsmClient::new().with_parameter("/app/config/database", "localhost:5432");
        ...
        assert_eq!(
            result.doc,
            serde_yaml::Value::String("localhost:5432".to_string())
        );
    ...
    async fn test_load_ssm_import_json() -> Result<()> {
        ...
        let client = MockSsmClient::new().with_parameter("/app/config/database", json_value);
    ...
    async fn test_load_ssm_import_yaml() -> Result<()> {
        ...
        let client = MockSsmClient::new().with_parameter("/app/config/database", yaml_value);
    ...
    async fn test_load_ssm_path_import() -> Result<()> {
        ...
        let client = MockSsmClient::new().with_path_parameters("/app/config", parameters);
