...
fn calculate_filehash(path: &str) -> Result<String, handlebars::RenderError> {
    ...
    if path.is_file() {
        ...
        Err(handlebars::RenderError::new(format!(
...
pub fn filehash_helper(
    ...
    let path_str = param.value().as_str().ok_or_else(|| {
        handlebars::RenderError::new("filehash helper requires a string parameter")
    })?;
...
pub fn filehash_base64_helper(
    ...
    let path_str = param.value().as_str().ok_or_else(|| {
        handlebars::RenderError::new("filehashBase64 helper requires a string parameter")
    })?;
