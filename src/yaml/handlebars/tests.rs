...
fn test_replace_helper() {
    ...
    let result =
        interpolate_handlebars_string("{{replace text \"hello\" \"hi\"}}", &env, "test").unwrap();
    assert_eq!(result, "hi world hi");

    // Test with no matches
    let result =
        interpolate_handlebars_string("{{replace text \"xyz\" \"abc\"}}", &env, "test").unwrap();
