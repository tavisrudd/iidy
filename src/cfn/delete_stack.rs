...
use crate::cfn::{
    DELETE_SUCCESS_STATES, determine_operation_success,
    stack_operations::{StackEventsService, StackInfoService, collect_stack_contents},
};
use crate::cli::{Cli, DeleteArgs};
use crate::output::{
    DynamicOutputManager, OutputData,
    aws_conversion::{
        convert_aws_error_to_error_info, convert_stack_events_to_display_with_max,
        convert_stack_to_definition, create_command_metadata, create_final_command_summary,
        get_caller_identity,
    },
...
async fn check_stack_exists_for_delete(
    context: &CfnContext,
    stack_name: &str,
...
async fn delete_stack_impl(
    ...
    let (stack, stack_id) = match check_stack_exists_for_delete(context, stack_name).await {
        ...
        Ok(None) => {
            ...
            output_manager
                .render(OutputData::StackAbsentInfo(stack_absent_info))
                .await?;
            let elapsed_seconds = context.elapsed_seconds().await?;
            let final_command_summary = create_final_command_summary(true, elapsed_seconds);
    ...
    let command_metadata = create_command_metadata(
        context,
        opts,
        &minimal_stack_args,
    ...
    let stack_id_for_deletion =
        perform_stack_deletion_without_output(context, stack_name, &stack_id, args).await?;
    ...
    let final_status = {
        ...
        .await
        {
            ...
            Err(error) => {
                let error_info =
                    convert_aws_error_to_error_info(&error, Some((context, cli))).await;
                output_manager.render(OutputData::Error(error_info)).await?;
                return Ok(1);
            }
        }
