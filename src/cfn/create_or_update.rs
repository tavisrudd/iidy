...
use crate::cfn::{
    CREATE_SUCCESS_STATES, CfnContext, CfnOperation, CfnRequestBuilder, StackArgs, StackChangeType,
    UPDATE_SUCCESS_STATES, UpdateResult, apply_stack_name_override_and_validate,
    changeset_operations::{self, check_stack_exists, confirm_changeset_execution},
    exec_changeset::call_exec_changeset_with_reconstruction,
    stack_operations::{StackInfoService, watch_stack_operation_and_summarize},
};
...
async fn create_or_update_impl(
    ...
    output_manager
        .render(OutputData::StackChangeDetails(stack_change_details.clone()))
        .await?;
