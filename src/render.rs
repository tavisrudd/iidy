...
use crate::{
    cli::RenderArgs,
    yaml::{engine::serialize_yaml_iidy_js_compatible, preprocess_yaml},
};
...
pub async fn handle_render_command(args: &RenderArgs) -> Result<()> {
    ...
    let processed_value =
        preprocess_yaml(&template_content, &base_location, &args.yaml_spec).await?;
...
pub fn apply_query_to_value(value: Value, query: &str) -> Result<Value> {
    ...
    for part in parts {
        ...
        match current {
            ...
            _ => {
                return Err(anyhow::anyhow!(
                    "Cannot query '{}' on non-mapping value",
                    part
                ));
            }
        }
    ...
    fn test_apply_query_to_value_not_found() {
        ...
        assert!(
            result
                .unwrap_err()
                .to_string()
                .contains("Query path 'NotFound' not found")
        );
    }
    ...
    fn test_apply_query_to_value_invalid_path() {
        ...
        assert!(
            result
                .unwrap_err()
                .to_string()
                .contains("Cannot query 'some' on non-mapping value")
        );
    }
}
