...
use iidy::yaml::imports::{ImportData, ImportLoader};
...
async fn test_error_reporting_shows_correct_input_uri() {
    ...
    let result = preprocessor
        .process(&file_content, &error_doc_path_str)
        .await;

    assert!(
        result.is_err(),
        "Processing should fail due to undefined variable"
    );
...
fn test_tagcontext_input_uri_direct_verification() {
    ...
    // Test with variables
    let context_with_vars =
        context.with_variable("env", serde_yaml::Value::String("prod".to_string()));
...
fn test_tagcontext_input_uri_propagation() {
    ...
    let with_var2 =
        with_var.with_variable("test2", serde_yaml::Value::String("value2".to_string()));
...
async fn test_import_chain_traceability() {
    ...
    let mut preprocessor = YamlPreprocessor::new(loader, false);
    let result = preprocessor
        .process(&file_content, &doc_a_path_str)
        .await
        .unwrap();
...
async fn test_multiple_imports_from_single_document() {
    ...
    let mut preprocessor = YamlPreprocessor::new(loader, false);
    let result = preprocessor
        .process(&file_content, &multi_import_path_str)
        .await;

    assert!(
        result.is_ok(),
        "Multi-import processing should succeed: {:?}",
        result.err()
    );
    let processed = result.unwrap();
    ...
    assert_eq!(
        db_connection, "db.example.com:3306",
    ...
    assert_eq!(
        combined_info, "DB=production_db Features=12",
...
"#,
    ...
    let mut preprocessor = YamlPreprocessor::new(loader, false);
    let result = preprocessor
        .process(&file_content, &complex_path_str)
        .await
        .unwrap();
...
async fn test_cycle_detection_working() {
    ...
    assert!(
        result.is_err(),
        "Cycle should be detected and processing should fail"
    );
...
async fn test_long_cycle_detection() {
    ...
    let mut preprocessor = YamlPreprocessor::new(loader, false);
    let result = preprocessor
        .process(&file_content, &long_cycle_a_path_str)
        .await;

    // Should fail with cycle detection error
    assert!(
        result.is_err(),
        "Long cycle should be detected and processing should fail"
    );
...
async fn test_self_import_cycle_detection() {
    ...
    let mut preprocessor = YamlPreprocessor::new(loader, false);
    let result = preprocessor
        .process(&file_content, &self_import_path_str)
        .await;

    // Should fail with cycle detection error
    assert!(
        result.is_err(),
        "Self-import cycle should be detected and processing should fail"
    );
...
"#;
    ...
    fs::write(base_path.join("self_import.yaml"), self_import_content)
        .await
        .unwrap();

    // Verify the file was created correctly
    let created_content = fs::read_to_string(base_path.join("self_import.yaml"))
        .await
        .unwrap();
    assert!(
        created_content.contains("./self_import.yaml"),
        "Self-import should reference itself"
    );
    assert!(
        created_content.contains("{{myself.data}}"),
        "Should have handlebars reference to self"
    );
...
async fn test_mixed_import_scenarios_design() {
    ...
    assert!(
        valid_config.exists(),
        "Valid config should exist for testing"
    );
    assert!(
        cycle_config.exists(),
        "Cycle config should exist for testing"
    );
...
async fn test_diamond_dependency_pattern() {
    ...
    let mut preprocessor = YamlPreprocessor::new(loader, false);
    let result = preprocessor
        .process(&file_content, &diamond_a_path_str)
        .await
        .unwrap();
    ...
    assert!(
        shared_from_left.contains("shared by both branches"),
        "Left path should access shared data"
    );
    assert!(
        shared_from_right.contains("shared by both branches"),
        "Right path should access shared data"
    );
...
async fn test_deep_nesting_import_chain() {
    ...
    let mut preprocessor = YamlPreprocessor::new(loader, false);
    let result = preprocessor
        .process(&file_content, &deep_a_path_str)
        .await
        .unwrap();
...
async fn test_large_fanout_pattern() {
    ...
    let mut preprocessor = YamlPreprocessor::new(loader, false);
    let result = preprocessor
        .process(&file_content, &fanout_a_path_str)
        .await
        .unwrap();
...
async fn test_cross_references_pattern() {
    ...
    let mut preprocessor = YamlPreprocessor::new(loader, false);
    let result = preprocessor
        .process(&file_content, &cross_a_path_str)
        .await
        .unwrap();
...
async fn test_dynamic_import_paths() {
    ...
    let mut preprocessor = YamlPreprocessor::new(loader, false);
    let result = preprocessor
        .process(&file_content, &dynamic_path_str)
        .await
        .unwrap();
...
async fn test_variable_shadowing_behavior() {
    ...
    let mut preprocessor = YamlPreprocessor::new(loader, false);
    let result = preprocessor
        .process(&file_content, &shadow_path_str)
        .await
        .unwrap();
...
"#,
    ...
    let mut preprocessor = YamlPreprocessor::new(loader, false);
    let result = preprocessor
        .process(&file_content, &empty_path.to_string_lossy())
        .await
        .unwrap();
...
"#,
    ...
    let mut preprocessor =
        YamlPreprocessor::new(UriTrackingImportLoader::new().await.unwrap(), false);
    let result = preprocessor
        .process(&file_content, &missing_path.to_string_lossy())
        .await;
...
"#,
    ...
    let mut preprocessor = YamlPreprocessor::new(loader, false);
    let result = preprocessor
        .process(&file_content, &conflict_path.to_string_lossy())
        .await;
...
"#,
    ...
    let mut preprocessor = YamlPreprocessor::new(loader, false);
    let result = preprocessor
        .process(&file_content, &unicode_path.to_string_lossy())
        .await;
    ...
    match result {
        Ok(processed) => {
            ...
            if let Some(special) = processed.get("special_result") {
                println!(
                    "   Special file result: {}",
                    special.as_str().unwrap_or("<non-string>")
                );
            }
            if let Some(emoji) = processed.get("emoji_result") {
                println!(
                    "   Emoji key result: {}",
                    emoji.as_str().unwrap_or("<non-string>")
                );
            }
        }
        Err(error) => {
...
"#,
    ...
    let result = preprocessor
        .process(&file_content, &stress_path.to_string_lossy())
        .await
        .unwrap();
    ...
    assert!(
        first_and_last.contains("stress test file 1"),
        "Should contain first file data"
    );
    assert!(
        first_and_last.contains("stress test file 20"),
