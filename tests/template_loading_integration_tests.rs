...
use iidy::cfn::template_loader::{load_cfn_template, load_cfn_stack_policy, TEMPLATE_MAX_BYTES};
...
async fn test_load_stack_policy_from_file() {
    ...
    let result = load_cfn_stack_policy(
        Some(&policy_value),
        temp_dir.path().to_str().unwrap(),
        None,
    ).await.unwrap();
...
async fn test_load_stack_policy_inline_yaml() {
    ...
    let policy_yaml = serde_yaml::from_str::<serde_yaml::Value>(r#"
Statement:
  - Effect: Deny
    Principal: "*"
