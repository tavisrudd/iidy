...
impl RendererTestUtils {
    ...
    pub fn normalize_output(output: &str) -> String {
        ...
        normalized = timestamp_re
            .replace_all(&normalized, "2024-01-15T10:30:45.123Z")
            .to_string();

        // Replace Unix timestamps with fixed placeholder
        let unix_timestamp_re = Regex::new(r"\b\d{10}\b").unwrap();
        normalized = unix_timestamp_re
            .replace_all(&normalized, "1705318245")
            .to_string();
        ...
        normalized = arn_re
            .replace_all(
                &normalized,
                "arn:aws:cloudformation:us-east-1:123456789012:stack/test-stack/test-id",
            )
            .to_string();
        ...
        normalized = account_re
            .replace_all(&normalized, "123456789012")
            .to_string();

        // Replace stack IDs with consistent test IDs
        let stack_id_re = Regex::new(r"stack/[^/]+/[a-f0-9-]+").unwrap();
        normalized = stack_id_re
            .replace_all(&normalized, "stack/test-stack/test-stack-id-123")
            .to_string();
...
Account: 987654321098
        ...
        assert!(
            normalized
                .contains("arn:aws:cloudformation:us-east-1:123456789012:stack/test-stack/test-id")
        );
    }
    ...
    fn test_validate_color_usage() {
        let output =
            "\x1b[31mERROR\x1b[0m: Stack creation failed\n\x1b[32mCREATE_COMPLETE\x1b[0m resource";
