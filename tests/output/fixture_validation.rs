...
async fn test_plain_renderer_against_fixture_expected_output() {
    ...
    assert!(
        ...
        "Fixture should have plain expected output"
    );
    let expected_plain = fixture.expected_outputs.get("plain").unwrap();

    // Verify the expected output has content
    assert!(
        !expected_plain.stdout.trim().is_empty(),
        "Expected plain stdout should not be empty"
    );
    assert_eq!(
        expected_plain.exit_code, 0,
        "Expected exit code should be 0"
    );
        ...
        match data {
            ...
            OutputData::StackContents(contents) => {
                renderer
                    ...
                    .await
                    .expect("Should render stack contents");
...
async fn test_interactive_renderer_against_fixture_expected_output() {
    ...
    assert!(
        ...
        "Fixture should have interactive expected output"
    );
    ...
    assert!(
        !expected_interactive.stdout.trim().is_empty(),
        "Expected interactive stdout should not be empty"
    );
    assert_eq!(
        expected_interactive.exit_code, 0,
        "Expected exit code should be 0"
    );
        ...
        match data {
            OutputData::CommandMetadata(metadata) => {
                renderer
                    .render_output_data(OutputData::CommandMetadata(metadata.clone()), None)
                    .await
                    .expect("Should render colored metadata");
            ...
            OutputData::StackDefinition(def, show_times) => {
                renderer
                    .render_output_data(OutputData::StackDefinition(def.clone(), *show_times), None)
                    .await
                    .expect("Should render colored stack definition");
            }
            OutputData::StackEvents(events) => {
                renderer
                    .render_output_data(OutputData::StackEvents(events.clone()), None)
                    .await
                    .expect("Should render colored stack events");
            }
            OutputData::StackContents(contents) => {
                renderer
                    .render_output_data(OutputData::StackContents(contents.clone()), None)
                    .await
                    .expect("Should render colored stack contents");
            }
            OutputData::CommandResult(result) => {
                renderer
                    .render_output_data(OutputData::CommandResult(result.clone()), None)
                    .await
                    .expect("Should render colored command result");
            }
            _ => {
                // Handle other types as needed
            }
...
async fn test_fixture_json_expected_output_structure() {
    ...
    assert!(
        ...
        "Fixture should have JSON expected output"
    );
    let expected_json = fixture.expected_outputs.get("json").unwrap();

    // Verify JSON output structure
    assert!(
        !expected_json.stdout.trim().is_empty(),
        "Expected JSON stdout should not be empty"
    );
    ...
    for line in lines {
        ...
        match type_name {
            "command_metadata" => {
                ...
                assert!(
                    data.get("cfn_operation").is_some(),
                    "Command metadata should have cfn_operation"
                );
                assert!(
                    data.get("region").is_some(),
                    "Command metadata should have region"
                );
                assert!(
                    data.get("primary_token").is_some(),
                    "Command metadata should have primary_token"
                );
            }
            "stack_definition" => {
                let data = json_value.get("data").unwrap();
                assert!(
                    ...
                    "Stack definition should have name"
                );
                assert!(
                    data.get("status").is_some(),
                    "Stack definition should have status"
                );
                assert!(
                    data.get("arn").is_some(),
                    "Stack definition should have arn"
                );
            }
            "command_result" => {
                let data = json_value.get("data").unwrap();
                assert!(
                    ...
                    "Command result should have success"
                );
                assert!(
                    data.get("elapsed_seconds").is_some(),
                    "Command result should have elapsed_seconds"
                );
                assert!(
                    data.get("exit_code").is_some(),
                    "Command result should have exit_code"
                );
            }
            _ => {
...
async fn test_fixture_color_expectations() {
    ...
    assert!(
        has_stack_definition,
        "Should have stack definition for coloring"
    );
    ...
    assert!(
        expected_interactive.stdout.len() > 1000,
        "Interactive output should be substantial"
    );
}
