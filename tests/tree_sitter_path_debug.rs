...
"#;
    ...
    if let Some(resources_node) = find_yaml_node_by_path(&root, &["Resources"], yaml_source) {
        println!(
            "Found Resources node: kind={}, text={:?}",
            resources_node.kind(),
            &yaml_source[resources_node.start_byte()
                ..resources_node
                    .end_byte()
                    .min(resources_node.start_byte() + 50)]
        );
    ...
    if let Some(bucket_node) =
        find_yaml_node_by_path(&root, &["Resources", "MyBucket"], yaml_source)
    {
        println!(
            "Found MyBucket node: kind={}, text={:?}",
            bucket_node.kind(),
            &yaml_source[bucket_node.start_byte()
                ..bucket_node.end_byte().min(bucket_node.start_byte() + 50)]
        );
    ...
    if let Some(bucket_name_node) = find_yaml_node_by_path(
        &root,
        &["Resources", "MyBucket", "Properties", "BucketName"],
        yaml_source,
    ) {
        println!(
            "Found BucketName node: kind={}, text={:?}",
            bucket_name_node.kind(),
            &yaml_source[bucket_name_node.start_byte()
                ..bucket_name_node
                    .end_byte()
                    .min(bucket_name_node.start_byte() + 100)]
        );
