...
async fn test_interactive_command_metadata_pixel_perfect() {
    ...
    let command_metadata = output_data.iter()
...
async fn test_plain_command_metadata_pixel_perfect() {
    ...
    let expected_plain = fixture.expected_outputs.get("plain")
    ...
    assert!(expected_output.contains("Command Metadata:"), "Expected output should have Command Metadata section");
    assert!(expected_output.contains("CFN Operation:"), "Expected output should have CFN Operation");
    assert!(expected_output.contains("create-stack"), "Expected output should have the operation name");
}
...
async fn test_interactive_stack_definition_pixel_perfect() {
    ...
    let (stack_definition, show_times) = output_data.iter()
        .find_map(|data| match data {
    ...
    let expected_output = &expected_interactive.stdout;
    assert!(expected_output.contains("Stack Details:"), "Expected output should have Stack Details section");
    assert!(expected_output.contains("Name:"), "Expected output should have Name field");
    assert!(expected_output.contains("Status:"), "Expected output should have Status field");
    assert!(expected_output.contains("test-stack"), "Expected output should have the stack name");
}
...
async fn test_interactive_stack_events_pixel_perfect() {
    ...
    let stack_events = output_data.iter()
        .find_map(|data| match data {
    ...
    let expected_output = &expected_interactive.stdout;
    assert!(expected_output.contains("Previous Stack Events"), "Expected output should have Stack Events section");
    assert!(expected_output.contains("Timestamp"), "Expected output should have Timestamp column");
    assert!(expected_output.contains("CREATE_COMPLETE"), "Expected output should have CREATE_COMPLETE status");
}
...
fn test_fixture_expected_output_completeness() {
    ...
    assert!(fixture.expected_outputs.contains_key("interactive"), "Should have interactive expected output");
    assert!(fixture.expected_outputs.contains_key("plain"), "Should have plain expected output");
    assert!(fixture.expected_outputs.contains_key("json"), "Should have json expected output");
    ...
    assert!(interactive.stdout.len() > 2000, "Interactive output should be substantial (got {} chars)", interactive.stdout.len());
    assert!(plain.stdout.len() > 2000, "Plain output should be substantial (got {} chars)", plain.stdout.len());
    assert!(json.stdout.len() > 1000, "JSON output should be substantial (got {} chars)", json.stdout.len());
    
    // Verify structure consistency
    assert_eq!(interactive.exit_code, 0, "Interactive should have exit code 0");
...
fn test_formatting_constants_match_iidy_js() {
    use iidy::output::renderers::interactive::{COLUMN2_START, MIN_STATUS_PADDING, MAX_PADDING, RESOURCE_TYPE_PADDING};
    
    // Verify constants match the implementation spec
    assert_eq!(COLUMN2_START, 25, "COLUMN2_START should match iidy-js spec");
    assert_eq!(MIN_STATUS_PADDING, 17, "MIN_STATUS_PADDING should match iidy-js spec"); 
    assert_eq!(MAX_PADDING, 60, "MAX_PADDING should match iidy-js spec");
    assert_eq!(RESOURCE_TYPE_PADDING, 40, "RESOURCE_TYPE_PADDING should match iidy-js spec");
}
