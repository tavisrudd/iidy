...
fn test_complex_yaml_path_building() {
    ...
    assert_eq!(
        deep_context.yaml_path,
        "Resources.Database[0].Properties.ConnectionStrings[2].Value"
    );
}
...
fn test_unicode_in_paths() {
    ...
    let unicode_context = context.with_path("资源").with_path("数据库");
...
fn test_context_cloning_behavior() {
    ...
    assert_eq!(
        context.file_location.as_ptr(),
        context1.file_location.as_ptr()
    );
}
