...
async fn test_debug_1_level_import() -> Result<()> {
    ...
    writeln!(level1_file, "  processed_value: \"{{level1_var}}-processed\"")?;
...
"#, level1_path);
    ...
    if let Value::Mapping(map) = &result {
        ...
        println!("  $defs present: {}", map.contains_key(&Value::String("$defs".to_string())));
        println!("  $imports present: {}", map.contains_key(&Value::String("$imports".to_string())));
...
async fn test_debug_2_level_import() -> Result<()> {
    ...
    writeln!(level1_file, "")?;
    writeln!(level1_file, "config:")?;
    writeln!(level1_file, "  level1_processed: \"{{level1_var}}-processed\"")?;
    writeln!(level1_file, "  level1_shared: \"{{shared_var}}\"")?;
    writeln!(level1_file, "  # Accessing level2 data")?;
    writeln!(level1_file, "  deep_access: !$ level2_data.deep_config.deep_value")?;
    let level1_path = level1_file.path().to_string_lossy().to_string();
...
"#, level1_path);
    ...
    if let Value::Mapping(map) = &result {
        ...
        if let Some(main_shared) = map.get(&Value::String("main_shared_check".to_string())) {
            println!("  main_shared_check: {:?} (should be 'MAIN_SHARED')", main_shared);
        }
        
        println!("\nLevel 1 data access:");
        if let Some(result_l1) = map.get(&Value::String("result_l1".to_string())) {
            println!("  result_l1: {:?} (should show L1_VALUE processing)", result_l1);
        }
        if let Some(result_l1_shared) = map.get(&Value::String("result_l1_shared".to_string())) {
            println!("  result_l1_shared: {:?} (should show L1_SHARED)", result_l1_shared);
        }
        
        println!("\nLevel 2 data access (via Level 1):");
        if let Some(result_l2) = map.get(&Value::String("result_l2_via_l1".to_string())) {
            println!("  result_l2_via_l1: {:?} (should show L2_VALUE processing)", result_l2);
        }
    }
...
async fn test_debug_3_level_import() -> Result<()> {
    ...
    writeln!(level1_file, "  level1_shared: \"{{shared_var}}\"")?;
    writeln!(level1_file, "  level2_access: !$ level2_data.deep_config.level2_value")?;
    writeln!(level1_file, "  level3_via_level2: !$ level2_data.deep_config.level3_access")?;
    let level1_path = level1_file.path().to_string_lossy().to_string();
