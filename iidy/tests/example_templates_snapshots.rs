...
async fn test_cloudformation_tags_structure() {
    ...
    assert!(result_str.contains("!Sub"), "Output should contain !Sub tags");
    assert!(result_str.contains("!Ref"), "Output should contain !Ref tags");
    assert!(result_str.contains("!GetAtt"), "Output should contain !GetAtt tags");
    
    // Verify that preprocessing variables were replaced
    assert!(result_str.contains("tag-demo-app"), "Preprocessing variables should be replaced");
    assert!(result_str.contains("production"), "Environment variables should be replaced");
    
    // Verify that CloudFormation variables are preserved
    assert!(result_str.contains("${AWS::Region}"), "CloudFormation variables should be preserved");
    assert!(result_str.contains("${AWS::AccountId}"), "CloudFormation variables should be preserved");
}
        ...
        if Path::new(&template_path).exists() {
            ...
            match result {
                Ok(_) => {
                    ...
                    eprintln!("Warning: {} unexpectedly succeeded - consider moving it to the valid examples", template);
                }
                Err(_) => {
...
async fn test_template_workflow_integration() {
    ...
    let _config_result = render_template_file("config.yaml").await
        .expect("Failed to render config.yaml");
    
    // 2. Then render the import-test file that imports config.yaml
    let import_result = render_template_file("import-test.yaml").await
        .expect("Failed to render import-test.yaml");
    
    // 3. Verify that the import worked correctly
    let import_str = serde_yaml::to_string(&import_result).expect("Failed to serialize import result");
    
    // Should contain values from the imported config
    assert!(import_str.contains("db.example.com"), "Should contain imported database host");
    assert!(import_str.contains("5432"), "Should contain imported database port");
    
    // Should also contain processed values
    assert!(import_str.contains("import-demo"), "Should contain processed app name");
}
...
"#;
    ...
    let result_str = serde_yaml::to_string(&result).expect("Failed to serialize result");
    assert!(result_str.contains("Monitoring: true"), "yes should convert to true");
    assert!(result_str.contains("EbsOptimized: false"), "no should convert to false");
    assert!(result_str.contains("SourceDestCheck: true"), "on should convert to true");
    assert!(result_str.contains("DisableApiTermination: false"), "off should convert to false");
}
...
mod performance_tests {
    ...
    async fn test_rendering_performance() {
        ...
        for template in &templates {
            let _ = render_template_file(template).await
