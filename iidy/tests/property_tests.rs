...
fn handlebars_template_strategy() -> impl Strategy<Value = String> {
    prop_oneof![
        ...
        (variable_name_strategy(), ".*").prop_map(|(var, text)| format!("{}{{{{{}}}}}suffix", text, var)), // Mixed content
    ]
}
...
proptest! {
    ...
    fn prop_yaml_parsing_scalars(value in yaml_scalar_strategy()) {
        ...
        let ast = parse_yaml_with_custom_tags(&yaml_str);
    ...
    fn prop_yaml_parsing_deterministic(
        ...
        let ast_result1 = parse_yaml_with_custom_tags(&yaml_content);
        let ast_result2 = parse_yaml_with_custom_tags(&yaml_content);
    ...
    fn prop_yaml_split_join_parsing(
        ...
        if !clean_parts.is_empty() {
            ...
            let ast = parse_yaml_with_custom_tags(&split_yaml);
    ...
    fn prop_yaml_parsing_graceful(malformed_yaml in ".*") {
        ...
        let parse_result = parse_yaml_with_custom_tags(&malformed_yaml);
...
mod standard_tests {
    ...
    fn test_yaml_scalar_strategy_generates_valid_values() {
        ...
        for _ in 0..10 {
            ...
            assert!(serialized.is_ok(), "Generated value should be serializable: {:?}", value);
        }
    }
