...
pub fn run_spinner_demo() {
    ...
    if !std::io::IsTerminal::is_terminal(&std::io::stdout()) {
        println!(
            "{}",
            "⚠ Not running in a TTY - spinners will show as text updates".warning()
        );
...
fn demonstrate_spinner_styles() {
    ...
    for (style, description) in styles {
        println!();
        let spinner = ProgressManager::with_style(
            style,
            &format!("Testing {} style...", format!("{:?}", style).muted()),
        );
        thread::sleep(Duration::from_millis(2000));
        spinner.succeed(&format!(
            "{} - {}",
            format!("{:?}", style).bold_text(),
            description
        ));
    }
}
...
fn demonstrate_ora_api() {
    ...
    let spinner =
        ProgressManager::with_style(SpinnerStyle::Arrow, "Gathering system information...");
...
fn simulate_stack_creation() {
    println!(
        "{}",
        "CloudFormation Stack Creation with Ora-like API:".bold_text()
    );
    println!();
