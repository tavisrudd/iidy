...
use crate::{
    aws,
    cli::{NormalizedAwsOpts, ExecChangeSetArgs},
...
pub async fn exec_changeset(opts: &NormalizedAwsOpts, args: &ExecChangeSetArgs) -> Result<()> {
    ...
    let (execute_request, token) = builder.build_execute_changeset(&args.changeset_name, "execute-changeset");
    reporter.show_step_token("execute-changeset", &token);
    
    reporter.show_progress(&format!("Executing changeset '{}' for stack: {}", 
        args.changeset_name, final_stack_args.stack_name.as_ref().unwrap()));
    ...
    if let Err(e) = watch_stack_with_context(&context, final_stack_args.stack_name.as_ref().unwrap(), 
