...
mod tests {
    ...
    async fn cfn_context_sets_start_time() {
        ...
        let ctx = CfnContext::new(client, time_provider, token_info).await.unwrap();
    ...
    async fn cfn_context_calculates_elapsed_time() {
        ...
        let mut ctx = CfnContext::new(client, time_provider.clone(), token_info).await.unwrap();
    ...
    async fn cfn_context_tracks_primary_token() {
        ...
        let ctx = CfnContext::new(client, time_provider, token_info.clone()).await.unwrap();
