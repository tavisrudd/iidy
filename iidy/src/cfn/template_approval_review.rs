...
async fn template_approval_review_impl(
    ...
    output_manager.render(OutputData::ApprovalStatus(approval_status)).await?;
    ...
    let latest_template = download_template(&s3_client, &bucket, "latest").await.unwrap_or_else(|_| String::new());
    ...
    output_manager.render(OutputData::TemplateDiff(template_diff)).await?;
    ...
    if !has_changes {
        ...
        output_manager.render(OutputData::ApprovalResult(result)).await?;
        return Ok(0);
    }
    ...
    output_manager.render(OutputData::ConfirmationPrompt(confirmation_request)).await?;
    ...
    if user_confirmed {
        // Approve template
        approve_template(&s3_client, &bucket, &pending_key, &approved_key, &pending_template).await?;
...
async fn approve_template(s3_client: &aws_sdk_s3::Client, bucket: &str, pending_key: &str, approved_key: &str, content: &str) -> Result<()> {
