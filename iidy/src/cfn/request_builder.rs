use aws_sdk_cloudformation::{
    types::{Parameter, Tag, Capability, OnFailure},
};
...
impl<'a> CfnRequestBuilder<'a> {
    ...
    pub fn build_create_stack(&self, step_name: &str) -> (aws_sdk_cloudformation::operation::create_stack::builders::CreateStackFluentBuilder, TokenInfo) {
        ...
        if let Some(ref capabilities) = self.stack_args.capabilities {
            let aws_capabilities: Vec<Capability> = capabilities.iter()
        ...
        if let Some(ref parameters) = self.stack_args.parameters {
            let aws_parameters: Vec<Parameter> = parameters.iter()
                .map(|(key, value)| {
                    Parameter::builder()
                        .parameter_key(key)
        ...
        if let Some(ref tags) = self.stack_args.tags {
            let aws_tags: Vec<Tag> = tags.iter()
                .map(|(key, value)| {
                    Tag::builder()
                        .key(key)
                        .value(value)
    ...
    pub fn build_update_stack(&self, step_name: &str) -> (aws_sdk_cloudformation::operation::update_stack::builders::UpdateStackFluentBuilder, TokenInfo) {
        let token = self.context.derive_token_for_step(step_name);
        
        let mut builder = self.context.client.update_stack()
            .client_request_token(&token.value);
    ...
    pub fn build_create_changeset(&self, changeset_name: &str, step_name: &str) -> (aws_sdk_cloudformation::operation::create_change_set::builders::CreateChangeSetFluentBuilder, TokenInfo) {
        ...
        let mut builder = self.context.client.create_change_set()
    ...
    pub fn build_execute_changeset(&self, changeset_name: &str, step_name: &str) -> (aws_sdk_cloudformation::operation::execute_change_set::builders::ExecuteChangeSetFluentBuilder, TokenInfo) {
        ...
        let mut builder = self.context.client.execute_change_set()
...
mod tests {
    ...
    fn mock_context() -> CfnContext {
        ...
        let token_info = TokenInfo::user_provided("test-token-123".to_string(), "test-op-1".to_string());
    ...
    fn mock_stack_args() -> StackArgs {
        ...
        StackArgs {
            ...
            tags: Some(tags),
            notification_arns: Some(vec!["arn:aws:sns:us-east-1:123456789012:test-topic".to_string()]),
    ...
    fn build_create_changeset_uses_correct_field_name() {
        ...
        let (_changeset_builder, token) = builder.build_create_changeset("test-changeset", "create-changeset");
    ...
    fn build_execute_changeset_uses_standard_field_name() {
        ...
        let (_execute_builder, token) = builder.build_execute_changeset("test-changeset-arn", "execute-changeset");
    ...
    fn different_steps_produce_different_tokens() {
        ...
        let (_, changeset_token) = builder.build_create_changeset("test-changeset", "create-changeset");
        let (_, execute_token) = builder.build_execute_changeset("test-changeset", "execute-changeset");
        
        // All tokens should be different
        let tokens = vec![&create_token.value, &update_token.value, &changeset_token.value, &execute_token.value];
        for (i, token1) in tokens.iter().enumerate() {
            for (j, token2) in tokens.iter().enumerate() {
                if i != j {
                    assert_ne!(token1, token2, "Tokens for different steps should be unique");
                }
            }
        }
