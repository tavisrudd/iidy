...
use crate::{
    aws,
    cli::{NormalizedAwsOpts, CreateChangeSetArgs},
...
pub async fn create_changeset(opts: &NormalizedAwsOpts, args: &CreateChangeSetArgs) -> Result<()> {
    ...
    let changeset_name = args.changeset_name.as_deref()
        .unwrap_or(&default_changeset_name);
    
    // Build and execute the CreateChangeSet request
    let (create_request, token) = builder.build_create_changeset(changeset_name, "create-changeset");
    reporter.show_step_token("create-changeset", &token);
    
    reporter.show_progress(&format!("Creating changeset '{}' for stack: {}", 
        changeset_name, final_stack_args.stack_name.as_ref().unwrap()));
