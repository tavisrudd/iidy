...
impl TagContext {
    ...
    pub fn add_scoped_variable(
        ...
        if let Some(ref mut scope_context) = self.scope_context {
            ...
            scope_context
                .current_scope
                .variables
                .insert(name.to_string(), scoped_var.clone());

            // Update the scope in the scopes map
            let scope_id = scope_context.current_scope.id.clone();
            scope_context
                .scopes
                .insert(scope_id, scope_context.current_scope.clone());
        }
    }
    ...
    pub fn create_import_scope(&mut self, import_key: &str, source_uri: &str) -> &mut Self {
        if let Some(ref mut scope_context) = self.scope_context {
            ...
            scope_context
                .scopes
                .insert(scope_id.clone(), import_scope.clone());
...
mod tests {
    ...
    fn test_tagcontext_input_uri_storage_s3_root_file() {
        ...
        assert!(context.input_uri.is_some());
        assert_eq!(context.input_uri.unwrap(), "s3://my-bucket/config.yaml");
        ...
        assert!(derived_base.is_some());
        assert_eq!(derived_base.unwrap().to_string_lossy(), "s3://my-bucket/");
    }
    ...
    fn test_tagcontext_input_uri_storage_https_urls() {
        ...
        // Test that we can derive base path on demand
        let derived_base =
            derive_base_path_from_location("https://example.com/configs/app/config.yaml");
        assert!(derived_base.is_some());
        assert_eq!(
            derived_base.unwrap().to_string_lossy(),
    ...
    fn test_tagcontext_input_uri_storage_http_urls() {
        ...
        // Test that we can derive base path on demand
        let derived_base =
            derive_base_path_from_location("http://example.com/configs/app/config.yaml");
        assert!(derived_base.is_some());
        assert_eq!(
            derived_base.unwrap().to_string_lossy(),
    ...
    fn test_tagcontext_comprehensive_relative_import_scenarios() {
        ...
        let test_cases = vec![
            ...
            (
                "s3://bucket/folder/subfolder/file.yaml",
                "s3://bucket/folder/subfolder/",
            ),
            // HTTPS cases
            ("https://example.com/file.yaml", "https://example.com/"),
            (
                "https://example.com/folder/file.yaml",
                "https://example.com/folder/",
            ),
            (
                "https://example.com/folder/subfolder/file.yaml",
                "https://example.com/folder/subfolder/",
            ),
            // HTTP cases
            ("http://example.com/file.yaml", "http://example.com/"),
            (
                "http://example.com/folder/file.yaml",
                "http://example.com/folder/",
            ),
            (
                "http://example.com/folder/subfolder/file.yaml",
    ...
    fn test_tagcontext_input_uri_storage_relative_path() {
        ...
        assert!(context.input_uri.is_some());
        assert_eq!(context.input_uri.unwrap(), "configs/app/config.yaml");

        // Test base path derivation
        let derived_base = derive_base_path_from_location("configs/app/config.yaml");
        assert!(derived_base.is_some());
        assert_eq!(derived_base.unwrap().to_string_lossy(), "configs/app");
    }
    ...
    fn test_tagcontext_input_uri_storage_native_paths() {
        ...
        #[cfg(windows)]
        assert!(
            base_path_str.contains("C:")
                && base_path_str.contains("configs")
                && base_path_str.contains("app")
        );
    }
    ...
    fn test_scoped_variable_addition_and_resolution() {
        ...
        assert_eq!(
            context.variables.get("env").unwrap(),
            &Value::String("production".to_string())
        );
        assert_eq!(
            context.variables.get("app_name").unwrap(),
            &Value::String("my-app".to_string())
        );
    ...
    fn test_variable_origin_tracking() {
        ...
        assert_eq!(
            context.get_variable_origin("local_var"),
            Some("local $defs".to_string())
        );
        assert_eq!(
            context.get_variable_origin("imported_var"),
            Some("imported from 'database'".to_string())
        );
        assert_eq!(
            context.get_variable_origin("tag_var"),
            Some("bound in !$let".to_string())
        );
    ...
    fn test_hierarchical_scope_creation() {
        ...
        if let Some(ref scope_context) = context.scope_context {
            ...
            assert_eq!(
                scope_context.current_scope.scope_type,
                ScopeType::ImportedDocument("config".to_string())
            );
    ...
    fn test_backward_compatibility() {
        ...
        context
            .variables
            .insert("test_var".to_string(), Value::String("test".to_string()));
