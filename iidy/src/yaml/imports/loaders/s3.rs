...
impl S3Client for AwsS3Client {
    async fn get_object(&self, bucket: &str, key: &str) -> Result<String> {
        ...
        let body = response.body.collect().await
            .map_err(|e| anyhow!("Failed to read S3 object body s3://{}/{}: {}", bucket, key, e))?;
        
        String::from_utf8(body.into_bytes().to_vec())
            .map_err(|e| anyhow!("S3 object s3://{}/{} contains invalid UTF-8: {}", bucket, key, e))
    }
}
...
pub async fn load_s3_import_with_client(location: &str, client: &dyn S3Client) -> Result<ImportData> {
...
mod tests {
    ...
    async fn test_load_s3_import_yaml() -> Result<()> {
        ...
        let client = MockS3Client::new()
            .expect_get_object("my-bucket", "config.yaml", Ok(yaml_content.to_string()));
    ...
    async fn test_load_s3_import_json() -> Result<()> {
        ...
        let client = MockS3Client::new()
            .expect_get_object("my-bucket", "config.json", Ok(json_content.to_string()));
        ...
        if let serde_yaml::Value::Mapping(map) = result.doc {
            assert_eq!(map.get(&serde_yaml::Value::String("key".to_string())), Some(&serde_yaml::Value::String("value".to_string())));
            assert_eq!(map.get(&serde_yaml::Value::String("number".to_string())), Some(&serde_yaml::Value::Number(serde_yaml::Number::from(42))));
        } else {
    ...
    async fn test_load_s3_import_text() -> Result<()> {
        ...
        let client = MockS3Client::new()
            .expect_get_object("my-bucket", "file.txt", Ok(text_content.to_string()));

        let result = load_s3_import_with_client("s3://my-bucket/file.txt", &client).await?;

        assert_eq!(result.import_type, ImportType::S3);
        assert_eq!(result.data, text_content);
        assert_eq!(result.doc, serde_yaml::Value::String(text_content.to_string()));
    ...
    async fn test_load_s3_import_error() {
        let client = MockS3Client::new()
            .expect_get_object("my-bucket", "nonexistent.yaml", Err(anyhow!("NoSuchKey")));
