...
fn test_thirteenth_if_tag_error_position() {
  ...
  Condition20: !$if { test: true, then: "ok20", else: "fail20" }"#;
    ...
    if error_msg.contains("multiple_if.yaml:2:") {
        println!(
            "❌ ISSUE CONFIRMED: Error incorrectly points to line 2 (first !$if) instead of line 14 (13th !$if with the actual error)"
        );
    } else if error_msg.contains("multiple_if.yaml:14:") {
        println!(
            "✅ ISSUE FIXED: Error correctly points to line 14 (13th !$if with the actual error)"
        );
    } else {
...
fn test_nested_if_tags_error_position() {
  ...
  OuterCondition: !$if
    ...
    if error_msg.contains("nested_if.yaml:2:") {
        println!(
            "❌ NESTED ISSUE: Error incorrectly points to line 2 (first !$if) instead of line 9 (nested !$if with error)"
        );
    } else if error_msg.contains("nested_if.yaml:9:") {
        println!(
            "✅ NESTED FIXED: Error correctly points to line 9 (nested !$if with the actual error)"
        );
    } else {
        println!(
            "🤔 Nested error points to some other location: {}",
            error_msg
        );
    }
}
