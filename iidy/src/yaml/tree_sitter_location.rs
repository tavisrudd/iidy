...
pub fn find_tag_in_node<'a>(node: &Node<'a>, tag_name: &str, source: &str) -> Option<Node<'a>> {
...
pub fn find_tag_position_with_tree_sitter(
    ...
    for (path_idx, &segment) in clean_path.iter().enumerate() {
        ...
        if let Some(&(_, array_index)) = index_positions
            .iter()
...
pub fn parse_path_with_indices<'a>(
...
"#;
        ...
        let result = find_tag_position_with_tree_sitter(
            yaml_source,
...
"#;
        ...
        println!(
            "Found third !$map at line {}, column {}",
            position.line, position.column
        );
    }
