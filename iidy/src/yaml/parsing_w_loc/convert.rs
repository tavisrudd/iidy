...
pub fn to_original_ast(ast: &with_location::YamlAst) -> original::YamlAst {
    match ast {
        ...
        with_location::YamlAst::TemplatedString(s, _) => {
            original::YamlAst::TemplatedString(s.clone())
        }
...
fn convert_cloudformation_tag(
    tag: &with_location::CloudFormationTag,
) -> original::CloudFormationTag {
    match tag {
    ...
    match tag {
        ...
        with_location::PreprocessingTag::If(if_tag) => {
            let converted = original::IfTag {
                ...
                else_value: if_tag
                    .else_value
                    .as_ref()
                    .map(|v| Box::new(to_original_ast(v))),
            };
        ...
        with_location::PreprocessingTag::Map(map_tag) => {
            let converted = original::MapTag {
                ...
                filter: map_tag
                    .filter
                    .as_ref()
                    .map(|f| Box::new(to_original_ast(f))),
            };
        ...
        with_location::PreprocessingTag::GroupBy(group_by_tag) => {
            let converted = original::GroupByTag {
                ...
                template: group_by_tag
                    .template
                    .as_ref()
                    .map(|t| Box::new(to_original_ast(t))),
            };
...
pub fn parse_and_convert_to_original(
    source: &str,
    uri_str: &str,
) -> anyhow::Result<original::YamlAst> {
        ...
        Err(_) => {
            ...
            match Url::from_file_path(uri_str) {
                ...
                Err(_) => {
                    ...
                    Url::parse(&format!("file://{}", uri_str)).map_err(|e| {
                        anyhow::anyhow!("Cannot create URI from '{}': {}", uri_str, e)
                    })?
                }
            }
        }
    ...
    let with_location_ast =
        parse_yaml_ast(source, uri).map_err(|e| anyhow::anyhow!("{}", e.message))?;

    Ok(to_original_ast(&with_location_ast))
}

/// New diagnostic API for convert module
pub fn parse_and_convert_to_original_with_diagnostics(
    source: &str,
    uri_str: &str,
...
fn create_uri_from_string(uri_str: &str) -> anyhow::Result<Url> {
    match Url::parse(uri_str) {
        ...
        Err(_) => match Url::from_file_path(uri_str) {
            Ok(uri) => Ok(uri),
            Err(_) => Url::parse(&format!("file://{}", uri_str))
