...
fn test_compatibility_with_example_templates() -> Result<()> {
    ...
    if !example_dir.exists() {
        eprintln!("Warning: Example directory {} does not exist, skipping compatibility tests", 
                 example_dir.display());
        return Ok(());
    }
...
fn test_compatibility_with_top_level_examples() -> Result<()> {
    ...
    if !example_dir.exists() {
        eprintln!("Warning: Example directory {} does not exist, skipping top-level compatibility tests", 
                 example_dir.display());
        return Ok(());
    }
...
fn test_compatibility_with_all_examples() -> Result<()> {
    ...
    if !example_dir.exists() {
        eprintln!("Warning: Example directory {} does not exist, skipping all examples compatibility tests", 
                 example_dir.display());
        return Ok(());
    }
    ...
    fn walk_directory(dir: &Path, total_files: &mut usize, successful_comparisons: &mut usize, failed_files: &mut Vec<(String, anyhow::Error)>) -> Result<()> {
    ...
    walk_directory(example_dir, &mut total_files, &mut successful_comparisons, &mut failed_files)?;
...
pub(crate) fn compare_asts(converted: &original_ast::YamlAst, original: &original_ast::YamlAst, file_path: &Path) -> Result<()> {
...
pub(crate) fn asts_equal(a: &original_ast::YamlAst, b: &original_ast::YamlAst) -> bool {
    match (a, b) {
        ...
        (original_ast::YamlAst::TemplatedString(a), original_ast::YamlAst::TemplatedString(b)) => a == b,
        ...
        (original_ast::YamlAst::Mapping(a), original_ast::YamlAst::Mapping(b)) => {
            ...
            a.iter().zip(b.iter()).all(|((k1, v1), (k2, v2))| {
                asts_equal(k1, k2) && asts_equal(v1, v2)
            })
        }
        
        (original_ast::YamlAst::PreprocessingTag(a), original_ast::YamlAst::PreprocessingTag(b)) => {
            preprocessing_tags_equal(a, b)
        }
        
        (original_ast::YamlAst::CloudFormationTag(a), original_ast::YamlAst::CloudFormationTag(b)) => {
        ...
        (original_ast::YamlAst::ImportedDocument(a), original_ast::YamlAst::ImportedDocument(b)) => {
...
fn preprocessing_tags_equal(a: &original_ast::PreprocessingTag, b: &original_ast::PreprocessingTag) -> bool {
    ...
    match (a, b) {
        ...
        (PreprocessingTag::Merge(a), PreprocessingTag::Merge(b)) => {
            ...
            a.sources.iter().zip(b.sources.iter()).all(|(x, y)| asts_equal(x, y))
        }
        ...
        (PreprocessingTag::Let(a), PreprocessingTag::Let(b)) => {
            ...
            a.bindings.iter().zip(b.bindings.iter()).all(|((k1, v1), (k2, v2))| {
                k1 == k2 && asts_equal(v1, v2)
            }) &&
...
fn cloudformation_tags_equal(a: &original_ast::CloudFormationTag, b: &original_ast::CloudFormationTag) -> bool {
    ...
    match (a, b) {
        ...
        (CloudFormationTag::ToJsonString(a), CloudFormationTag::ToJsonString(b)) => asts_equal(a, b),
...
"#;
        ...
        match parse_yaml_ast(nested_yaml, uri.clone()) {
            Ok(ast) => {
                ...
                match original_parser::parse_yaml_with_custom_tags_from_file(nested_yaml, uri.as_str()) {
                    Ok(original_ast) => {
        ...
        if path.exists() {
            ...
            match test_single_file_compatibility(path) {
                ...
                Err(_e) => {
                    ...
                    let original_ast = original_parser::parse_yaml_with_custom_tags_from_file(&content, file_uri.as_str()).unwrap();
                    let converted_ast = to_original_ast(&tree_sitter_ast);
                    
                    // Look for first structural difference in a smaller file
                    if let (original_ast::YamlAst::Mapping(conv_pairs), original_ast::YamlAst::Mapping(orig_pairs)) = (&converted_ast, &original_ast) {
                        println!("Converted has {} pairs, Original has {} pairs", conv_pairs.len(), orig_pairs.len());
                        
                        // Check first 3 key-value pairs for differences
                        for i in 0..std::cmp::min(3, std::cmp::min(conv_pairs.len(), orig_pairs.len())) {
                            let (conv_key, conv_val) = &conv_pairs[i];
                            let (orig_key, orig_val) = &orig_pairs[i];
                            
                            if !asts_equal(conv_key, orig_key) {
                                println!("Key difference at position {}: Conv={:?}, Orig={:?}", i, conv_key, orig_key);
                            } else if !asts_equal(conv_val, orig_val) {
                                if let original_ast::YamlAst::PlainString(key_name) = conv_key {
                                    println!("Value difference for key '{}' at position {}", key_name, i);
                                    println!("  Conv val type: {:?}", std::mem::discriminant(conv_val));
                                    println!("  Orig val type: {:?}", std::mem::discriminant(orig_val));
                                }
                            } else if let original_ast::YamlAst::PlainString(conv_str) = conv_key {
                                println!("Position {}: Key '{}' and value match perfectly", i, conv_str);
                            }
                        }
                    }
                }
...
"#;
        ...
        match parse_yaml_ast(multiline_yaml, uri.clone()) {
            Ok(ast) => {
                ...
                // Parse with original implementation
                match original_parser::parse_yaml_with_custom_tags_from_file(multiline_yaml, uri.as_str()) {
                    Ok(original_ast) => {
                        println!("✓ Original parser succeeded");
                        
                        if compare_asts(&converted, &original_ast, Path::new("multiline_test")).is_ok() {
...
"#;
        ...
        match parse_yaml_ast(resources_yaml, uri.clone()) {
            Ok(ast) => {
                ...
                match original_parser::parse_yaml_with_custom_tags_from_file(resources_yaml, uri.as_str()) {
                    Ok(original_ast) => {
                        println!("✓ Original parser succeeded");
                        
                        if compare_asts(&converted, &original_ast, Path::new("resources_test")).is_ok() {
...
"#;
        ...
        match parse_yaml_ast(eq_test_yaml, uri.clone()) {
            Ok(ast) => {
                ...
                match original_parser::parse_yaml_with_custom_tags_from_file(eq_test_yaml, uri.as_str()) {
                    Ok(original_ast) => {
